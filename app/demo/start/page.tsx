...
import { saveCard } from "@/lib/halo/store";
import { buildMockCard } from "@/lib/halo/mockCard";
...
export default function Page() {
  ...
  // Id of the durable, persisted canonical card backing /card, /map, /share.
  ...
  // Generate + persist the durable canonical Moment Card (Past / Present /
  // Future) that powers /card, /map and /share. Uses /api/generate-card (real
  // model when OPENAI_API_KEY is set, deterministic mock otherwise); on any
  // network failure it builds the mock locally so a card is always saved.
  async function persistCanonicalCard(memoryText: string, followUpAnswers: string[]) {
    try {
      const r = await fetch("/api/generate-card", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ memoryText, followUpAnswers, entryLabel: seed.entryLabel }),
      });
      if (!r.ok) throw new Error(`generate-card ${r.status}`);
      const data = (await r.json()) as { card?: MomentCard };
      if (!data.card) throw new Error("no card");
      saveCard(data.card);
      setSavedCardId(data.card.id);
    } catch {
      const card = buildMockCard(memoryText, followUpAnswers, { entryLabel: seed.entryLabel });
      saveCard(card);
  ...
  async function goToResult() {
    // Derive the memory fragment + follow-up answers from the live transcript,
    // falling back to the seed so the card is saved even on the preset path.
    const liveUser = liveMessages
      .filter((m) => m.role === "user")
      .map((m) => m.text.trim())
      .filter(Boolean);
    const seedUser = seed.transcript
      .filter((m) => m.role === "user")
      .map((m) => m.text.trim())
      .filter(Boolean);
    const source = liveUser.length > 0 ? liveUser : seedUser;
    const memoryText = source[0] ?? seed.userFragment ?? "";
    const followUpAnswers = source.slice(1);
    // Fire-and-forget: persistence must never block or break the result screen.
    void persistCanonicalCard(memoryText, followUpAnswers);
  ...
  return (
    <ResultPage
      ...
      onRestart={reset}
      savedCardId={savedCardId ?? undefined}
    />
  );
}
